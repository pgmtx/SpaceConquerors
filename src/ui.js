...
export function refreshSelectedPlanet(mapCells) {
	...
	const cell = mapCells.find(
		(c) => c.planete?.idPlanete === _lastPlanete.idPlanete,
	);
...
export function showPlanetInfo(planete) {
  ...
  `;
	...
	const actionsButtons = document.getElementById("actions-buttons");
	const isOurs =
		planete.proprietaire?.idEquipe === state.teamId ||
...
function renderModulesPanel(container, planete) {
	...
	if (available.length === 0 && placed.length === 0) {
		container.innerHTML =
			'<span style="color:#557799;font-size:11px">Aucun module disponible</span>';
		return;
	}
