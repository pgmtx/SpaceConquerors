...
// ── Team color palette ────────────────────────────────────────
const TEAM_COLORS = [
	0x00ffcc, 0xff4466, 0x4488ff, 0xff8800, 0xaa44ff, 0x00ff88, 0xff2288,
	0x44ccff, 0xffff00, 0xff6600, 0x88ff00, 0xff0088,
];
...
export function getTeamColor(teamId) {
	...
	const color =
		teamId === state.teamId
			? 0x00ffcc
			: TEAM_COLORS[(idx + 1) % TEAM_COLORS.length];
	...
	model.traverse((child) => {
		if (child.isMesh && child.material) {
			...
			child.material.emissiveIntensity =
				vaisseau.proprietaire === state.teamId ? 0.3 : 0.1;
		}
	});
...
export function getClickedObject(raycaster) {
	...
	cellObjects.forEach(({ group }) => {
		group.children.forEach((c) => {
			if (c.userData.isPlanet) planetMeshes.push(c);
		});
	});
	...
	cellObjects.forEach(({ group }) => {
		group.children.forEach((c) => {
			if (c.userData.isTile) tiles.push(c);
		});
	});
