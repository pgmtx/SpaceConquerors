...
async function refreshAllTeams() {
	try {
		...
		state.myTeam =
			state.allTeams.find((t) => t.idEquipe === state.teamId) || null;
		if (state.myTeam) {
			// positionX/Y absents de /equipes, forcer proprietaire pour les boutons
			if (myShips)
				state.myTeam.vaisseaux = myShips.map((s) => ({
					...s,
					proprietaire: state.teamId,
				}));
...
function selectShipByIndex(index) {
	...
	if (vaisseau.positionX !== undefined) {
		const newX = Math.max(
			...
			Math.min(
				58 - state.viewSize,
				vaisseau.positionX - Math.floor(state.viewSize / 2),
			),
		);
		const newY = Math.max(
			0,
			Math.min(
				58 - state.viewSize,
				vaisseau.positionY - Math.floor(state.viewSize / 2),
			),
		);
		state.viewX = newX;
		state.viewY = newY;
	...
	notify(
		`Vaisseau ${shipSelectIndex + 1}/${ships.length} : ${vaisseau.nom}`,
		"info",
	);
}
		...
		.addEventListener("click", async (e) => {
			...
			if (state.pendingAction) {
				...
				if (hit.type === "cell") {
					...
					cx = hit.data?.positionX;
					cy = hit.data?.positionY;
				}
				if (cx !== undefined) {
...
function handleKeyMovement(delta) {
	...
	if (Math.abs(moveAccum.x) >= 1) {
		...
		state.viewX = Math.max(
			0,
			Math.min(58 - state.viewSize, state.viewX + step),
		);
		moveAccum.x -= step;
	...
	if (Math.abs(moveAccum.y) >= 1) {
		...
		state.viewY = Math.max(
			0,
			Math.min(58 - state.viewSize, state.viewY + step),
		);
		moveAccum.y -= step;
		panCameraTo(state.viewX, state.viewY);
		scheduleMapRefresh();
